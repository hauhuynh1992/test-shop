...
import com.example.quiz.utils.ShopHopConstants.AssetFiles.ATTRIBUTES
import com.example.quiz.utils.ShopHopConstants.AssetFiles.CATEGORYS
import com.example.quiz.utils.ShopHopConstants.AssetFiles.PRODUCTS
import com.example.quiz.utils.ShopHopConstants.AssetFiles.REVIEWS
...
fun addUserReview(reviewData: ShopHopRequestModel, onApiSuccess: (Boolean) -> Unit) {
    ...
    if (isExist(reviewData.product_id!!.toInt(), list)) {
        ...
        productReviewData.date_created = ShopHopConstants.FULL_DATE_FORMATTER.format(Date())
        list.add(productReviewData)
    }
    getSharedPrefInstance().setValue(ShopHopConstants.SharedPref.USER_REVIEWS, Gson().toJson(list))
    onApiSuccess(true)
}
...
fun getUserReviews(): ArrayList<ProductReviewData> {
    return if (getSharedPrefInstance().getStringValue(ShopHopConstants.SharedPref.USER_REVIEWS).isEmpty()) {
        ArrayList()
    } else {
        Gson().fromJson(
            getSharedPrefInstance().getStringValue(ShopHopConstants.SharedPref.USER_REVIEWS, ""),
            object : TypeToken<ArrayList<ProductReviewData>>() {}.type
        )
    }
}
...
fun getWishlist(): ArrayList<WishList> {
    return if (getSharedPrefInstance().getStringValue(ShopHopConstants.SharedPref.WISHLIST_DATA).isEmpty()) {
        ArrayList()
    } else {
        Gson().fromJson(
            getSharedPrefInstance().getStringValue(ShopHopConstants.SharedPref.WISHLIST_DATA, ""),
...
fun Activity.updateWishList(list: ArrayList<WishList>) {
    getSharedPrefInstance().setValue(ShopHopConstants.SharedPref.WISHLIST_DATA, Gson().toJson(list))
    getSharedPrefInstance().setValue(ShopHopConstants.SharedPref.KEY_WISHLIST_COUNT, list.size)
...
fun getOrders(): ArrayList<MyOrderData> {
    return if (getSharedPrefInstance().getStringValue(ShopHopConstants.SharedPref.KEY_ORDERS).isEmpty()) {
        ArrayList()
    } else {
        Gson().fromJson(
            getSharedPrefInstance().getStringValue(ShopHopConstants.SharedPref.KEY_ORDERS, ""),
...
fun Activity.addOrder(myOrderData: MyOrderData) {
    ...
    getSharedPrefInstance().setValue(ShopHopConstants.SharedPref.KEY_ORDERS, Gson().toJson(list))
    getSharedPrefInstance().setValue(ShopHopConstants.SharedPref.KEY_ORDER_COUNT, list.size)
